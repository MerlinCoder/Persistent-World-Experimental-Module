    ...
    (else_try), # section of events received by server from the clients
      ...
      (else_try), # handle admins changing the server password
        ...
        (try_begin),
          ...
          (server_set_password, s0),
        (try_end),
	  #TWENTY PERCENT BEGIN
      (else_try),
		(eq, ":event_type", client_event_drop_twenty_percent),
		(player_get_gold, ":gold", ":sender_player_id"),
		(store_div, ":drop_amount", ":gold", 5),
		(try_begin),
		   (gt, ":drop_amount", 0),
		   (call_script, "script_cf_drop_money_bag_item", ":sender_player_id", ":drop_amount"),
		   (str_store_player_username, s11, ":sender_player_id"),
		   (str_store_string, s10, "@{s11} dropped twenty percent!"),
		(else_try),
			(str_store_string, s10, "@{s11} has no money to drop!"),
		(try_end),	
		...
		(try_for_agents, ":other_agent_id"), # send the chat message to other players whoose agents are close enough
		  ...
		  (position_move_z, pos2, 160),
		  (get_sq_distance_between_positions, ":sq_distance", pos1, pos2),
		  (le, ":sq_distance", max_distance_local_chat),
		  (this_or_next|le, ":sq_distance", ambient_distance_local_chat),
		  (position_has_line_of_sight_to_position, pos1, pos2),
		  (multiplayer_send_string_to_player, ":other_player_id", ":server_event", s10),
		(try_end),	
	  #TWENTY PERCENT END
